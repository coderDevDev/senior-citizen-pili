...
export default function BASCAActivityLogsPage() {
    ...
    try {
      ...
      if (userData.user) {
        ...
        if (user?.barangay) {
          ...
          setFilters(prev => ({ ...prev, barangay: user.barangay }));
        }
      }
    } catch (error) {
  ...
  const handleExport = async (format: 'csv' | 'json') => {
    try {
      ...
      if (format === 'csv') {
        ...
        filename = `activity-logs-${userBarangay}-${
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#666666] mt-2">
            Track activities in {userBarangay} barangay
          </p>
        </div>
