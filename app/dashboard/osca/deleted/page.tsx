...
export default function OSCARecentlyDeletedPage() {
  ...
  const loadDeletedItems = async () => {
    ...
    try {
      ...
      const { data: documents } = await supabase
        ...
        .select(
          '*, deleted_by_user:users!document_requests_deleted_by_fkey(first_name, last_name), senior_citizens(first_name, last_name)'
        )
        .not('deleted_at', 'is', null)
        .order('deleted_at', { ascending: false });
      ...
      const { data: appointments } = await supabase
        .from('appointments')
        .select(
          '*, deleted_by_user:users!appointments_deleted_by_fkey(first_name, last_name), senior_citizens(first_name, last_name)'
        )
        .not('deleted_at', 'is', null)
        .order('deleted_at', { ascending: false });
      ...
      const { data: benefits } = await supabase
        ...
        .select(
          '*, deleted_by_user:users!benefit_applications_deleted_by_fkey(first_name, last_name), senior_citizens(first_name, last_name)'
        )
        .not('deleted_at', 'is', null)
        .order('deleted_at', { ascending: false });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[#666666] mt-2">Restore or permanently delete items</p>
      </div>
